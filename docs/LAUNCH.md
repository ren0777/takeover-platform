...
`compose.yaml` provides PostgreSQL, an explicit migration job, API and web services. The database and API are internal; the web listener binds to loopback for a separately configured HTTPS reverse proxy. Configure a domain, TLS, ingress request/connection limits, alert routing, and backups before public traffic.
...
6. Apply the reviewed deterministic territory seed once using `docker compose exec api pnpm --filter @takeover/database db:seed:territories`. This creates no fictional owners.
