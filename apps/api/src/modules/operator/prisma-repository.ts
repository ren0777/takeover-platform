...
export class OperatorMutationConflictError extends Error {
  ...
  constructor() {
    super('Record changed since operator review');
    this.name = 'OperatorMutationConflictError';
  }
}
...
export class OperatorRecordNotFoundError extends Error {
  ...
  constructor() {
    super('Operator target not found');
    this.name = 'OperatorRecordNotFoundError';
  }
}
...
export class PrismaOperatorRepository implements OperatorRepository {
  ...
  inspectTarget(
    ...
    if (kind === 'recovery-requests')
      return this.prisma.companyAccessRequest.findUnique({
        where: { id },
        include: {
          company: { select: { id: true, name: true, status: true } },
          contact: { select: { id: true, email: true, emailVerifiedAt: true, revokedAt: true } },
        },
      });
    if (kind === 'companies')
    ...
    if (kind === 'territories')
      return this.prisma.territory.findUnique({
        where: { id },
        select: { id: true, name: true, availabilityStatus: true, version: true, updatedAt: true },
      });
    return this.prisma.companyManagementGrant.findUnique({
      where: { id },
      select: {
        id: true,
        companyId: true,
  ...
  listManagementGrants(limit: number): Promise<unknown[]> {
    return this.prisma.companyManagementGrant.findMany({
      take: limit,
      orderBy: [{ updatedAt: 'desc' }, { id: 'asc' }],
      select: {
        id: true,
  ...
  listReconciliationActions(limit: number): Promise<unknown[]> {
    return this.prisma.paymentReconciliationAction.findMany({
      orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
      take: limit,
    });
  }
  ...
  ): Promise<{ items: Prisma.CompanyAccessRequestGetPayload<true>[]; nextCursor: string | null }> {
    const cursorParts = options.cursor?.split('|');
    const cursor = cursorParts
      ? { createdAt: new Date(cursorParts[0]!), id: cursorParts[1]! }
      : undefined;
    const rows = await this.prisma.companyAccessRequest.findMany({
      where: {
        ...(options.scope === 'actionable'
          ? {
              recoveryStatus: 'PENDING' as const,
              recoveryExpiresAt: { gt: options.now },
              status: { in: ['PENDING', 'EXPIRED'] as Array<'PENDING' | 'EXPIRED'> },
            }
          : { recoveryStatus: { not: 'NONE' as const } }),
        ...(cursor
          ? {
              OR: [
  ...
  async setCompanySuspended(input: MutationInput & { suspended: boolean }): Promise<unknown> {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (before.updatedAt.getTime() !== input.expectedUpdatedAt.getTime())
        throw new OperatorMutationConflictError();
      const target = input.suspended ? 'SUSPENDED' : 'ACTIVE';
      if (input.suspended ? before.status !== 'ACTIVE' : before.status !== 'SUSPENDED')
        throw new OperatorMutationConflictError();
      const changed = await tx.company.updateMany({
        where: { id: input.id, updatedAt: input.expectedUpdatedAt, status: before.status },
        data: { status: target },
      });
      ...
      await tx.auditLog.create({
        data: this.audit(
          input,
          `operator.company.${input.suspended ? 'suspended' : 'restored'}`,
          'company',
          before.id,
          before.id,
  ...
  async setTerritoryDisabled(input: MutationInput & { disabled: boolean }): Promise<unknown> {
    return this.prisma.$transaction(async (tx) => {
      ...
      const changed = await tx.territory.updateMany({
        where: {
          id: input.id,
          updatedAt: input.expectedUpdatedAt,
          availabilityStatus: before.availabilityStatus,
        },
        data: { availabilityStatus: target, version: { increment: 1 } },
      });
      if (changed.count !== 1) throw new OperatorMutationConflictError();
      ...
      await tx.auditLog.create({
        data: this.audit(
          input,
          `operator.territory.${input.disabled ? 'disabled' : 'enabled'}`,
          'territory',
          before.id,
          null,
          {
  ...
  async revokeManagementGrant(input: MutationInput): Promise<unknown> {
    return this.prisma.$transaction(async (tx) => {
      ...
      const changed = await tx.companyManagementGrant.updateMany({
        where: { id: input.id, updatedAt: input.expectedUpdatedAt, status: 'ACTIVE' },
        data: { status: 'REVOKED', revokedAt },
      });
      if (changed.count !== 1) throw new OperatorMutationConflictError();
      const after = await tx.companyManagementGrant.findUniqueOrThrow({ where: { id: input.id } });
      await tx.companyManagementSession.updateMany({
        where: { grantId: input.id, revokedAt: null },
        data: { revokedAt },
      });
      await tx.auditLog.create({
        data: this.audit(
          input,
          'operator.management_grant.revoked',
          'company_management_grant',
          before.id,
          before.companyId,
          {
            before: { status: before.status, revokedAt: before.revokedAt },
  ...
  async decideRecoveryRequest(
    ...
    return this.prisma
      .$transaction(
        async (tx) => {
          await tx.$queryRaw(
            Prisma.sql`SELECT "id" FROM "company_access_requests" WHERE "id" = ${input.id}::uuid FOR UPDATE`,
          );
          const before = await tx.companyAccessRequest.findUnique({ where: { id: input.id } });
          if (before === null) throw new OperatorRecordNotFoundError();
          if (
            before.updatedAt.getTime() !== input.expectedUpdatedAt.getTime() ||
            before.recoveryStatus !== 'PENDING' ||
            before.recoveryExpiresAt === null ||
            before.recoveryExpiresAt <= input.now ||
            !['PENDING', 'EXPIRED'].includes(before.status)
          )
            throw new OperatorMutationConflictError();
          if (input.decision === 'approve') {
            ...
            const grant = await tx.companyManagementGrant.upsert({
              where: {
                companyId_contactId: { companyId: before.companyId, contactId: before.contactId },
              },
              create: {
                companyId: before.companyId,
                contactId: before.contactId,
                source: 'MANUAL_RECOVERY',
                accessRequestId: before.id,
                grantedAt: input.now,
              },
              update: {
                status: 'ACTIVE',
            ...
            await tx.emailVerificationChallenge.updateMany({
              where: { companyId: before.companyId, contactId: before.contactId, revokedAt: null },
              data: { revokedAt: input.now },
            });
          ...
          if (before.takeoverIntentId !== null) {
            const intent = await tx.takeoverIntent.findUnique({
              where: { id: before.takeoverIntentId },
            });
            if (
              intent &&
              intent.companyId === before.companyId &&
          ...
          await tx.auditLog.create({
            data: this.audit(
              input,
              `operator.company_recovery.${input.decision === 'approve' ? 'approved' : 'rejected'}`,
              ...
              before.companyId,
              {
                before: {
                  ...
                  recoveryStatus: before.recoveryStatus,
                  updatedAt: before.updatedAt,
                },
                after: {
                  status: after.status,
                  recoveryStatus: after.recoveryStatus,
