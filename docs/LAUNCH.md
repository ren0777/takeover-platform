...
3. Configure Dodo **test** API key, Standard Webhooks secret and JSON currency/product mapping. Checkout is offered (quotes report `checkoutAvailable: true` and `POST /api/takeover-checkouts` proceeds) only when a provider is configured **and** `PAYMENTS_ENABLED=true`; with either missing, every quote says checkout is unavailable and nothing can be charged. Set `PAYMENTS_ENABLED=true` against the **test** base URL for the sandbox matrix. `DODO_LIVE_ENABLED` is a separate gate gating the live provider host and stays `false` until live charging is authorized. Subscribe the provider to the public `/api/payment/webhooks/dodo` URL. Run the sandbox matrix below. The software does not create products or send test payments automatically.
