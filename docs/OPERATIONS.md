...
Company, territory and management-grant targets are available at `/api/operator/companies`, `/api/operator/territories`, `/api/operator/management-grants` and their `/:id` detail routes. Each exposes the exact `updatedAt` for reviewed mutations. Other read endpoints are bounded to 100 newest rows: `/api/operator/audit-logs`, `/api/operator/payments`, and `/api/operator/reconciliation-actions`. These inspect state only. They do not edit amounts, payment status, ownership, or provider reconciliation claims.
...
Available state transitions are company `suspend`/`restore`, territory `disable`/`enable`, management-grant `revoke`, and recovery-request `decision`. Grant revocation also revokes all sessions for that grant. Recovery decisions accept `decision: approve|reject` only for an unexpired `PENDING` recovery at the reviewed version, with an original access-request status of `PENDING` or `EXPIRED`. The independent recovery deadline governs the decision even when the original access deadline has passed. Approval grants management access to the verified requester and does not revoke other managers, alter payments, or transfer territory ownership. Review identity evidence and an external support ticket before approval.
...
Recovery reads use `/api/operator/recovery-requests?limit=50` (1�100 rows per page). The default `scope=actionable` includes pending recoveries whose recovery deadline is still future and whose original request is `PENDING` or `EXPIRED`. Requests can still be rejected if the contact or company no longer qualifies for approval. Expired recovery windows and already decided requests cannot block this queue.

The response retains the `data` array and adds `meta.nextCursor` (null at the end). Pass the URL-encoded cursor as `cursor` with the same scope and limit to fetch the next page. Ordering is oldest access-request `createdAt` first, then UUID; these immutable keys keep later pages stable when earlier requests are decided or removed. This is a live queue, not a frozen snapshot: newly actionable requests before the current cursor appear after restarting from the first page.

Use `scope=all` with the same pagination to inspect recovery history, including expired and resolved rows. `/api/operator/recovery-requests/:id` reads an exact request regardless of queue eligibility, including its current `updatedAt`, recovery deadline, company status, and contact verification/revocation state. Both queue and detail require `read` permission; decisions require `moderate`. Fetch detail and review evidence immediately before deciding.
